...
- Publish to clicomp bus via BaseChannel._handle_message()
...
from clicomp.config.schema import Base
from clicomp.security.network import validate_url_target

try:
    from clicomp.config.paths import get_media_dir
...
def _make_bot_class(channel: QQChannel) -> type[botpy.Client]:
    ...
    class _Bot(botpy.Client):
        def __init__(self):
            # Disable botpy's file log — clicomp uses loguru; default "botpy.log" fails on read-only fs
...
class QQConfig(Base):
    ...
    # Optional: directory to save inbound attachments. If empty, use clicomp get_media_dir("qq").
    media_dir: str = ""
    ...
    def _init_media_root(self) -> Path:
        ...
        if self.config.media_dir:
            ...
            try:
                ...
                root = Path.home() / ".clicomp" / "media" / "qq"
        else:
            root = Path.home() / ".clicomp" / "media" / "qq"
