...
from clicomp.config.paths import get_media_dir, get_runtime_subdir
from clicomp.config.schema import Base
from clicomp.utils.helpers import split_message
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.clicomp/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'clicomp channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"clicomp-{uuid.uuid4().hex[:12]}"
