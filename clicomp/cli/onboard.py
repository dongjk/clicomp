"""Interactive onboarding questionnaire for clicomp."""
...
from clicomp.cli.models import (
    format_token_count,
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]clicomp[{__version__}][/bold cyan]")
    )
    console.print()
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from clicomp.config.schema to get
    ...
    from clicomp.config.schema import AgentDefaults
...
# Channel support has been removed. Keep onboarding focused on core settings only.
